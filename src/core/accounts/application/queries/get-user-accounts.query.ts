...
export class GetUserAccounts {
  ...
  async execute(query: GetUserAccountsQuery): Promise<Account[]> {
    ...
    const activeMembers = members.filter((member) => !member.isDisabled() && !member.isPending());
    ...
    const accounts = await Promise.all(accountIds.map((id) => this.accountRepository.findById(id)));
